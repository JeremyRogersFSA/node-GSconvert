...
const filterAndFormatEntries = (listOfEntries, entry) => {
  ...
  if (!isWithdrawn) {
    const abs = Number(entry.absencePlusPartial)
    const ex = Number(entry.excusedAbsences)
    ...
    for (const key in entry) {
      if (key.indexOf('/') > 0 && Date.parse(key) < Date.now()) {
        ...
        if (entry[key] === 'Tardy') newObj.partial.push(key)
...
const createDraft = async (raw, gmail, cohort) => {
  try {
...
const generateEmails = async (entries) => {
  ...
  const itmEmails = {
    '2302-ACC-ET-WEB-PT-A': `<samuel.pizarro${domain}>, <michael.pascuzzi${domain}>, <Frank.Chambergo${domain}>, <Thomas.Pollick${domain}>`,
    '2302-ACC-ET-WEB-PT-B': `<samuel.pizarro${domain}>, <charles.lucas${domain}>, <Aiesha.Brown${domain}>, <Woody.lucas${domain}>`,
    ...
    '2302-ACC-ET-WEB-PT-E': `<samuel.pizarro${domain}>, <tony.shaker${domain}>, <Abi.scholz${domain}>, <Jing.cao${domain}>`,
    '2302-ACC-CT-WEB-PT-A': `<samuel.pizarro${domain}>, <Aaron.shen${domain}>, <Gonca.ay${domain}>, <Kemp.atkinson${domain}>`,
    '2302-ACC-CT-WEB-PT-B': `<samuel.pizarro${domain}>, <jacqueline.levine${domain}>, <Caitlin.lamprecht${domain}>, <Elijah.hensel${domain}>`,
    '2302-ACC-PT-WEB-PT-A': `<samuel.pizarro${domain}>, <John.sickels${domain}>, <Abidur.dipta${domain}>, <Jonathan.martinez${domain}>`,
    '2302-ACC-PT-WEB-PT-B': `<samuel.pizarro${domain}>, <Aaron.katz${domain}>, <Danielle.williams${domain}>, <David.boicourt${domain}>`,
  ...
  for (const entry of entries) {
    ...
    const { sName, email, cohort, absences, absent, partial } = entry
    ...
    if (isOverAbsences) {
      ...
      const message = [
        ...
        `Hello ${sName},\n\nI hope you're doing well. I noticed that you've been out out these dates\n\n${absent.join(
          '\n'
        )}\n\n${
          partial.length ? 'You also have partial days of ' + partial.join(', ') + '\n\n' : ''
      ...
      const message = [
        ...
        `Hello ${sName},\n\nI hope you're doing well. I noticed that you were out\n\n${absent.join(
          '\n'
        )}\n\n${
          partial.length ? 'You also have partial days of ' + partial.join(', ') + '\n\n' : ''
